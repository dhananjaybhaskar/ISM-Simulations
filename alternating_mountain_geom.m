...
visualize_curvature_heatmap(X,0,vis_x,vis_y,vis_z,mesh_x,mesh_y,mesh_z, [-5 30], [-5 30], [-10 10], G_color_limits, G_curvature, false);
visualize_curvature_heatmap(X,1,vis_x,vis_y,vis_z,mesh_x,mesh_y,mesh_z, [-5 30], [-5 30], [-10 10], M_color_limits, M_curvature, false);
pause;
...
function [curvature] = gaussian_curvature_alternating_mountain(Theta_mesh_fine, Phi_mesh_fine, q)
    ...
    assert(min(min(B.^2 - 4 .* A .* C)) > 0); %if not, something has gone horribly wrong
